...
func findmax_rate(L Lagu, n int, i int) int {
	var Angkamin float64
	var max int
	Angkamin = L.banyak.sumRate[i]
	max = i
	for j := i + 1; j < n; j++ {
		if L.banyak.sumRate[j] > Angkamin {
			Angkamin = L.banyak.sumRate[j]
			max = j
		}
	}
	return max
...
func cari_judul(L *Lagu) {
	...
	for i = 0; i < NMAX; i++ {
		if judul == L.judul[i] && artis == L.artist[i] {
			sedang_memainkan_lagu(L, i)
			ketemu++
		}
	}
...
func explore(L *Lagu) {
	...
	for pilihan != 4 {
		...
		fmt.Println("==  3. Hapus Semua Daftar Putar                       ==")
...
func reset_dafput(L *Lagu) {
	var simpan_PL, total int
	total = Play_list.banyak_PL
	for i := 0; i <= total; i++ {
		simpan_PL = banyak_lagu_di_setiap_PL[i]
		for j := 0; j <= simpan_PL; j++ {
			Play_list.nama_PL[i] = Play_list.nama_PL[i+1]
			Play_list.PL_ke[i].artist[j] = Play_list.PL_ke[i+1].artist[j]
			Play_list.PL_ke[i].judul[j] = Play_list.PL_ke[i+1].judul[j]
			Play_list.PL_ke[i].genre[j] = Play_list.PL_ke[i+1].genre[j]
			Play_list.PL_ke[i].banyak.sumListener[j] = Play_list.PL_ke[i+1].banyak.sumListener[j]
			Play_list.PL_ke[i].banyak.sumRate[j] = Play_list.PL_ke[i+1].banyak.sumRate[j]
			Play_list.PL_ke[i].banyak.sumLikes[j] = Play_list.PL_ke[i+1].banyak.sumLikes[j]
			Play_list.PL_ke[i].banyak.sumDislikes[j] = Play_list.PL_ke[i+1].banyak.sumDislikes[j]
			Play_list.PL_ke[i].banyak.sumViews[j] = Play_list.PL_ke[i+1].banyak.sumViews[j]
			Play_list.PL_ke[i].banyak.sumLikes[j] = Play_list.PL_ke[i+1].banyak.sumLikes[j]
			if banyak_lagu_di_setiap_PL[i] > 0 {
				banyak_lagu_di_setiap_PL[i]--
			}
		}
		Play_list.banyak_PL--
	}
...
func sorting_rate(L *Lagu) {
	var idx, i int
	for i = 0; i < L.banyak.sumLagu; i++ {
		idx = findmax_rate(*L, L.banyak.sumLagu, i)
		swap_Lagu(L, idx, i)
	}
}
...
func sorting_hits(L *Lagu) {
	var idx, i int
	for i = 0; i < L.banyak.sumLagu; i++ {
		idx = findmax_hits(*L, L.banyak.sumLagu, i)
		swap_Lagu(L, idx, i)
	}
}
func findmax_hits(L Lagu, n int, i int) int {
	var Angkamin int
	var max int
	Angkamin = L.banyak.sumListener[i]
	max = i
	for i := i + 1; i < n; i++ {
		if L.banyak.sumListener[i] > Angkamin {
			Angkamin = L.banyak.sumListener[i]
			max = i
		}
	}
